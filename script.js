(function () {
  ...
  function stopJani(options = {}) {
    ...
    JarvisUI.setTranscript(options.message || "Jani stopped. Tap mic to arm wake mode.");
    if (options.addMessage) {
      JarvisUI.addMessage("system", options.message || "Jani stopped.");
    }
  }
  ...
  function boot() {
    ...
    document.getElementById("stopSpeechButton").addEventListener("click", () => {
      stopJani({
        addMessage: true,
        message: "Jani stopped and reset.",
      });
    });
  }
