(function () {
  const cannedReplies = [
    "Systems are nominal. Jarvis is listening.",
  ...
  const wakePattern = /\b(hey\s+jarvis|ok\s+jarvis|jarvis|jarvis\s+please)\b/i;
  ...
  function stripWakeWords(input) {
    return input
      .replace(/^(hey\s+jarvis|ok\s+jarvis|jarvis)\b[:,]?\s*/i, "")
      .replace(/\s+\b(hey\s+jarvis|ok\s+jarvis|jarvis|assistant)\b$/i, "")
      .replace(/^(hey|ok)\s+/i, "")
      .replace(/\s+/g, " ")
      .trim();
  }
  ...
  function getWakeIntent(rawText) {
    ...
    const leadingWake = text.match(/^(?:hey\s+jarvis|ok\s+jarvis|jarvis)\b[:,]?\s*(.*)$/i);
    if (leadingWake) {
      return {
        hasWakeWord: true,
        commandText: stripWakeWords(leadingWake[1] || ""),
        heardText: text,
      };
    }

    const trailingWake = text.match(/^(.*?)\s+\b(jarvis)\b$/i);
    if (trailingWake) {
      return {
        hasWakeWord: true,
  ...
  function getUtilityCommand(text) {
    ...
    if (/(who are you|your name|tum kaun)/i.test(lower)) {
      return {
        action: "chat",
        response: "I am Jarvis, a Chrome-based voice assistant running fully in this browser.",
      };
    }
    ...
    if (/(salam|assalam|hello|hi|hey)/i.test(lower)) {
      return {
        action: "chat",
        response: "Wa alaikum assalam. Jarvis online and at your service.",
      };
    }
  ...
  function processCommand(rawText) {
    ...
    if (!text) {
      return {
        action: "chat",
        response: "Jarvis is awake. Say a command after the wake phrase.",
      };
    }
